...
export function AssetIcon({ assetId, symbol, size = "md", className = "" }: AssetIconProps) {
  ...
  if (isLpToken) {
    ...
    return (
      <div className={`flex items-center justify-center bg-ink-100 dark:bg-dark-border rounded-full text-ink-600 dark:text-dark-text-muted font-medium ${getSizeClasses(size)} ${className}`}>
        {symbol.slice(0, 2)}
      </div>
    );
  }
  ...
  if (logo?.png && !imageError) {
    return (
      <img
        ...
        onError={() => {
          console.log(`Failed to load image for ${symbol} (${assetId}):`, logo.png);
          setImageError(true);
        }}
        onLoad={() => {
...
function LPTokenIcon({ pair, size, className }: { pair: LPTokenPair; size: "sm" | "md" | "lg"; className: string }) {
  ...
  const containerSize = size === "sm" ? "w-7 h-4" : size === "md" ? "w-9 h-6" : "w-11 h-8";
  const iconSize = size === "sm" ? "w-4 h-4" : size === "md" ? "w-6 h-6" : "w-8 h-8";
  const textSize = size === "sm" ? "text-xs" : size === "md" ? "text-sm" : "text-base";
  const overlap = size === "sm" ? "right-0" : size === "md" ? "right-0" : "right-0";
    ...
    <div className={`relative ${containerSize} ${className}`}>
      ...
      <div className="absolute left-0 top-0 z-10">
        {token1Logo?.png ? (
          ...
          <div className={`flex items-center justify-center bg-ink-100 dark:bg-dark-border rounded-full text-ink-600 dark:text-dark-text-muted font-medium ${iconSize} ${textSize} border-2 border-white dark:border-dark-surface shadow-sm`}>
